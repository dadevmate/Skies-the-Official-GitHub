...
class PostsModel: ObservableObject {
    ...
    func addData(username: String, post: String, date: String, reported: Bool, imageURL: String, pfp: String, verified: Bool, subscription: String, timestamp: Int) {
        
        // Get a reference to the database
        let db = Firestore.firestore()
        // Add a document to a collection
        db.collection("posts").addDocument(data: ["username": username, "post": post, "date": date, "reported": reported, "imageURL": imageURL, "pfp": pfp, "verified": verified, "subscription": subscription, "timestamp": timestamp]) { error in
                ...
                if let snapshot = snapshot { // check is snapshot is nil
                    ...
                    // it assigns all the documents to the variable, list, which saved it in an array
                      ...
                      // Create a Todo item for each document returned
                            return PostsData(id: doc.documentID, username: doc["username"] as? String ?? "", post: doc["post"] as? String ?? "", date: doc["date"] as? String ?? "", imageURL: doc["imageURL"] as? String ?? "", reported: doc["reported"] as? Bool ?? false, pfp: doc["pfp"] as? String ?? "", verified: doc["verified"] as? Bool ?? false, subscription: doc["subscription"] as? String ?? "", timestamp: doc["timestamp"] as? Int ?? 0)
                    }
                    }
                }
